...
/**
 * Formal covering note that accompanies the shared PDF, identical on WhatsApp
 * and email: same letter structure, and no sign-off on either. The mail
 * template already appends the company signature, and on WhatsApp the details
 * are in the attachment anyway — so the note closes on the order line rather
 * than repeating the address and GSTIN.
 *
